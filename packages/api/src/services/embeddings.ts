...
export namespace EmbeddingsService {
  ...
  export const layer = (config: Config): Layer.Layer<Service, never, HttpClient.HttpClient> =>
    Layer.effect(
      Service,
      Effect.gen(function* createService() {
        ...
        const generate = Effect.fn("EmbeddingsService.generate")(function* generate(
          ...
          const request = yield* HttpClientRequest.post(`${config.baseUrl}/v1/embeddings`).pipe(
        ...
        return Service.of({
          isEnabled: () => !!(config.baseUrl && config.apiToken),
          generate,
          generateText,
        });
      }),
  ...
  export const defaultLayer = (config: Config): Layer.Layer<Service> =>
