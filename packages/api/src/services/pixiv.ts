import { BookmarkRestrict, PixivClient, parseNextUrl } from "@book000/pixivts";
...
export interface PixivBookmarkPage {
...
const mapArtwork = (illust: PixivIllustItem): PixivArtwork => ({
  ...
  mediaUrls:
    illust.metaPages.length > 0
      ...
      : [illust.metaSinglePage.originalImageUrl].filter((url): url is string => typeof url === "string"),
});
...
export class PixivAdapter {
  ...
  async bookmarks(options: { cursor?: number; visibility: "public" | "private" }): Promise<PixivBookmarkPage> {
    const result = await this.#client.users.bookmarks.illusts({
      userId: this.#client.userId,
      restrict: options.visibility === "private" ? BookmarkRestrict.PRIVATE : BookmarkRestrict.PUBLIC,
      maxBookmarkId: options.cursor,
    });
    if (result.isErr) {
      throw new Error(`Pixiv bookmark request failed: ${result.error.type}`);
    }
    const next = result.value.nextUrl ? parseNextUrl(result.value.nextUrl).maxBookmarkId : undefined;
    return {
