...
const transformPostsBase = <T extends TransformPost>(posts: T[], getMedia: (post: T) => TransformMedia[]): PostData[] =>
  posts.map((post) => {
...
export const transformPosts = (posts: (TransformPost & { media: (TransformMedia & { s3Url?: string })[] })[]) =>
