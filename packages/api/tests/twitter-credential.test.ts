...
describe("Twitter credential service", () => {
  ...
  let concurrentAction: "delete" | "save" | null;

  beforeEach(() => {
    credential = {
      credentialType: "cookies",
      encryptedSecret: `legacy:${cookiePayload}`,
      provider: "twitter",
      user: { telegramId },
    };
  ...
  const createService = () =>
    createTwitterCredentialService({
      ...
      decrypt: (secret) => {
        ...
        return {
          data: secret.replace(/^(?<prefix>legacy|scoped):/u, ""),
          usedLegacyEncryption: !secret.startsWith("scoped:"),
        };
      },
      encrypt: (value) => `scoped:${value}`,
  ...
  test("upgrades legacy encryption with compare-and-swap", async () => {
    expect(await createService().get(userId)).toBe(cookiePayload);
    expect(credential?.encryptedSecret).toBe(`scoped:${cookiePayload}`);
  });

  test("preserves valid plaintext legacy cookie JSON", async () => {
    if (credential) {
      credential.encryptedSecret = cookiePayload;
    }
...
describe("Pixiv credential service", () => {
  ...
  const withPixivClient = createPixivCredentialService({
    ...
    find: () => Promise.resolve({ credentialType: "refresh_token", encryptedSecret: "original-token" }),
  ...
  test("does not overwrite a concurrently changed credential", async () => {
    ...
    await expect(withPixivClient("user", operation)).rejects.toThrow("Pixiv credential changed during token rotation");
    expect(operation).not.toHaveBeenCalled();
  });
});
