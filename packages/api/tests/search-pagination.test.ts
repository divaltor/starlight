...
import { isAfterSearchCursor, isSearchCursorPayload, Cursor, SearchCursorPayloadSchema } from "../src/utils/cursor";
import type { SearchCursorPayload } from "../src/utils/cursor";
...
describe("search post pagination", () => {
  ...
  test("does not create a cursor when the page contains exactly the limit", () => {
    const page = paginateSearchResults([row("post-1", "photo-1", 0.9), row("post-1", "photo-2", 0.9)], 1);
...
describe("search cursor ordering", () => {
  test("matches score/provider/post/user descending lexicographic order", () => {
    expect(isAfterSearchCursor({ finalScore: 0.7, provider: "z", postId: "99", userId: "z" }, cursor)).toBe(true);
    expect(isAfterSearchCursor({ finalScore: 0.8, provider: "pixiv", postId: "99", userId: "z" }, cursor)).toBe(true);
    expect(isAfterSearchCursor({ finalScore: 0.8, provider: "twitter", postId: "10", userId: "a" }, cursor)).toBe(true);
    expect(isAfterSearchCursor({ finalScore: 0.8, provider: "twitter", postId: "09", userId: "z" }, cursor)).toBe(true);
    expect(isAfterSearchCursor({ finalScore: 0.8, provider: "twitter", postId: "10", userId: "c" }, cursor)).toBe(
      false,
    );
    expect(isAfterSearchCursor({ finalScore: 0.8, provider: "twitter", postId: "10", userId: "b" }, cursor)).toBe(
      false,
    );
  });

  test("rejects obsolete incomplete cursors", () => {
    expect(isSearchCursorPayload({ lastScore: 1, lastPhotoId: "1", queryTime: cursor.queryTime })).toBe(false);
  });
