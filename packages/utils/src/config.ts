...
const env = createEnv({
  server: {
    ...
    BOT_ALIASES: z
      ...
      .transform((value) => [...new Set(value.split(",").map((alias) => alias.trim().toLowerCase()))].filter(Boolean)),
    ...
    CLOUDFLARE_WORKERS_AI_SUMMARY_MODEL: z.string().default("@cf/meta/llama-3.2-11b-vision-instruct"),
