...
import { bytesToHex, bytesToUtf8, hexToBytes, managedNonce, randomBytes, utf8ToBytes } from "@noble/ciphers/utils.js";
...
export class CookieEncryption {
  ...
  constructor(masterKey: string | Uint8Array, salt?: string | Uint8Array) {
    ...
    this.salt = typeof salt === "string" ? utf8ToBytes(salt) : (salt ?? utf8ToBytes("starlight-cookie-salt-v1"));
  }
  ...
  safeDecrypt(data: string, userId: string): string {
    ...
    try {
      ...
      throw new Error(`Failed to decrypt cookie data: ${error instanceof Error ? error.message : "Unknown error"}`, {
        cause: error,
      });
    }
  }
