...
export default defineConfig({
  ...
  rules: {
    ...
    // idiom, not a Node-style callback. Wildcard imports stay allowed only
    // for external packages (`import * as React from "react"` in shadcn/ui).
    ...
    // module, pair a value with a same-name type, export a module-local
    // `Error` union, and pass anonymous generators to `Effect.gen`.
    // Static-method classes are also used as namespaces for utility APIs
    // (`Attachment.save`, `History.build`). Module grouping itself is done
    // with `export namespace <Name>` blocks consumed via named imports, so
    // namespace members are module-level declarations, not nested blocks.
    ...
    // OpenCode V2-style module grouping: modules declare `export namespace X`
    // and consumers import it by name. Star imports of internal modules are
    // forbidden; apps/web keeps `import * as React` (shadcn/ui convention).
