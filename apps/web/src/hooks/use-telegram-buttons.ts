...
import type { ButtonAction, ButtonState, MainButtonConfig, RouteButtonConfig } from "@/types/telegram-buttons";
...
export function useTelegramButtons(
  ...
  const updateButtonsInternal = (config: RouteButtonConfig) => {
    ...
    if (config.secondaryButton) {
      syncBottomButton(config.secondaryButton, "secondaryButton", executeButtonAction, registerCleanup, {
