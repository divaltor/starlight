...
function AlertDialog({ open, onOpenChange, children }: AlertDialogProps) {
  ...
  return <AlertDialogContext.Provider value={contextValue}>{open ? children : null}</AlertDialogContext.Provider>;
}
...
function AlertDialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return <div className={cn("flex flex-col-reverse gap-2 sm:flex sm:flex-row sm:justify-end", className)} {...props} />;
}

function AlertDialogTitle({ className, ...props }: React.ComponentProps<typeof Heading>) {
  return (
    <Heading className={cn("font-semibold text-lg leading-none tracking-tight", className)} slot="title" {...props} />
  );
}

function AlertDialogDescription({ className, ...props }: React.ComponentProps<"p">) {
  return <p className={cn("text-base-content/60 text-sm", className)} slot="description" {...props} />;
}
...
function AlertDialogCancel({ className, variant = "outline", ...props }: React.ComponentProps<typeof Button>) {
  ...
  return <Button variant={variant} className={className} onClick={() => onOpenChange(false)} {...props} />;
}
