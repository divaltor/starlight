...
type QuickManagerRead = { cookies: Cookie[]; state: "matched" } | { state: "json-other" } | { state: "not-json" };
...
export function decodeCookies(value: string | null, cookieNames?: string[]): Cookie[] | null {
  ...
  try {
    ...
    if (decodedText !== null) {
      return resolveCookieAttempt(decodedText, readQuickManagerExport(decodedText, targetNames), targetNames);
    }
