...
export interface Interface {
	readonly generate: <OUTPUT>(
		input: GenerateInput<OUTPUT>,
	) => Effect.Effect<GenerationResult<OUTPUT>, Error>;
}

export class Service extends Context.Service<Service, Interface>()("starlight/Model") {}

export const layer: Layer.Layer<Service, never, ModelProvider.Service | ModelTelemetry.Service> =
	Layer.effect(
		Service,
		Effect.gen(function* layer() {
			...
			const generate = Effect.fn("Model.generate")(function* generate<OUTPUT>(
				input: GenerateInput<OUTPUT>,
			) {
...
function clampOutputTokens(value: number | undefined): number {
	return Math.min(
		Math.max(value ?? selected.limits.defaultOutputTokens, 1),
...
function createToolEvent(event: {
	...
	const input = Predicate.isObject(event.toolOutput.input)
		? structuredClone(event.toolOutput.input)
		: {};
	if (event.toolOutput.type === "tool-result") {
		return {
			durationMs: event.toolExecutionMs,
			input,
			output: Predicate.isObject(event.toolOutput.output)
				? structuredClone(event.toolOutput.output)
				: {},
			state: "completed",
...
function logFailure(
	...
	const stepUsage = steps.map((step) =>
		normalizeStepUsage(step.usage, step.providerMetadata, selected.prices),
	);
	const usage = aggregateGenerationUsage(stepUsage);
	// Dot notation is the project convention; destructuring is intentionally disabled.
	...
	return Effect.all([
		...steps.map((step, index) =>
			Effect.sync(() => recordStep(telemetry, step, stepUsage[index]!)),
		),
		Effect.sync(() =>
