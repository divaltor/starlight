...
test("does not put prompt or tool-result content in model logs", async () => {
	...
	expect(
		logs.find((entry) => entry.message === "Model generation completed")?.annotations,
	).toMatchObject({ billingInputTokens: 20, stepCount: 2 });
...
interface ModelTestInput<OUTPUT> {
	...
	readonly tools?: readonly Model.Tool[];
}
...
function runModelEffect<OUTPUT>(
	...
	return Effect.gen(function* () {
		...
		Effect.provide(
			Model.layer.pipe(Layer.provide(Layer.merge(ModelProvider.testLayer(model), telemetry))),
		),
	);
}
