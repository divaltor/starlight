...
import { aggregateGenerationUsage, normalizeStepUsage } from "@/ai/usage";

test("keeps missing cache detail unknown", () => {
	expect(normalizeStepUsage(createUsage({}), undefined, selected.prices)).toMatchObject({
...
test("excludes inconsistent cache usage from cost thresholds", () => {
	...
	expect(aggregateGenerationUsage([usage]).validForCostThresholds).toBe(false);
});

test("keeps usage unknown when an invocation records no provider step", () => {
	expect(aggregateGenerationUsage([])).toEqual({
		billing: {
			cacheReadTokens: null,
