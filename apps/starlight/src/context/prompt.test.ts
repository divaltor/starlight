...
test("appending a finalized turn preserves every prior context segment", () => {
	...
	const base = new Bun.CryptoHasher("sha256")
		...
		.digest("hex");
	const first = Prompt.extendPrefix(
		base,
		Prompt.renderTurn({ content: "Alice: hello", role: "user" }),
	);
	...
	expect(
		Prompt.extendPrefix(base, Prompt.renderTurn({ content: "Alice: hello", role: "user" })),
	).toEqual(first);
