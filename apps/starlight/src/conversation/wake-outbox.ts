...
export class Service extends Context.Service<Service, Interface>()(
	"starlight/ConversationWakeOutbox",
) {}

export const layer: Layer.Layer<Service, never, Database.Service | WakeQueue.Service> =
	Layer.effect(
		Service,
		Effect.gen(function* layer() {
			...
			const publishAvailable = Effect.fn("ConversationWakeOutbox.publishAvailable")(
				function* publishAvailable(limit: number) {
					const now = new Date();
					...
					yield* Effect.all(
						stranded.flatMap((row) => {
							if (
								row.lane.activeRunId === null &&
								row.lane.pendingRevision <= row.lane.processedRevision
							) {
								return [];
							}
							if (
								row.lane.activeRunId !== null &&
								(row.lane.leaseUntil === null || row.lane.leaseUntil > now)
							) {
								return [];
							}
							return [
...
export const publisherLayer: Layer.Layer<never, never, Service> = Layer.effectDiscard(
	Effect.gen(function* publisherLayer() {
		...
		const publish = outbox.publishAvailable(100).pipe(
			...
			Effect.catch((error) =>
				Effect.logError("Conversation wake publication failed").pipe(
