...
export class ConversationError extends Schema.TaggedError<ConversationError>()(
	"ConversationError",
	{
		cause: Schema.optional(Schema.Defect()),
		message: Schema.String,
	...
	Effect.gen(function* layer() {
			...
			const admitted = yield* database
				.transaction(async (transaction) => {
					...
					const rawMessage = {
						...
						editDate:
							input.payload.editDate === null ? null : new Date(input.payload.editDate * 1000),
					...
					const created = await transaction.conversationInput.create({
						data: {
							...
							replyToMessageId: input.payload.replyToMessageId,
							senderTelegramId:
								input.payload.senderId === null ? null : BigInt(input.payload.senderId),
							sourceMessageId: input.payload.messageId,
							sourceRevision,
		...
		const drain = Effect.fn("Conversation.drain")(function* drain(input: LaneWakeInput) {
			...
			return yield* Effect.gen(function* drainClaimed() {
				...
				const generated = yield* invokeModel(database, claimed, prepared, model, exa);
				if (generated === null) return { kind: "completed" as const, runId: claimed.runId };
...
export class OptionsService extends Context.Service<OptionsService, Options>()(
	"starlight/ConversationOptions",
) {}
...
function prepareRun(
	...
	return Effect.gen(function* prepare() {
		...
		const prepared = {
			...
			sessionId: yield* Effect.promise(() =>
				ConversationKey.affinity(claimed.key, options.affinitySecret),
			),
				...
				await transaction.conversationRun.update({
					...
					data: {
						...
						requestHash: new Bun.CryptoHasher("sha256")
							.update(Prompt.canonicalEncode(request))
							.digest("hex"),
					},
				});
			})
		...
		.transaction(async (transaction) => {
			...
			await transaction.conversationToolCall.createMany({
				data: generated.toolEvents.map((event) => ({
					...
					result:
						event.state === "completed" ? (event.output as Prisma.InputJsonObject) : undefined,
					runId: claimed.runId,
			...
			await transaction.conversationRunAction.createMany({
				data: generated.output.replies.map((action, ordinal) => {
					...
					const invalidTarget =
						target !== null && target !== undefined && !allowedTargets.has(target);
...
function recordModelFailure(
	...
	return database
		.transaction(async (transaction) => {
			...
			const hasSuccessor = lane.pendingRevision > claimed.inputEndRevision;
			const nextWakeAt = terminal && hasSuccessor ? new Date() : null;
			await transaction.conversationRun.update({
				where: { id: claimed.runId },
				data: {
			...
			await transaction.conversationLane.update({
				...
				data: terminal
					? {
							activeRunId: null,
							firstPendingAt: hasSuccessor ? new Date() : null,
							leaseOwner: null,
							leaseUntil: null,
							nextWakeAt,
							processedRevision: claimed.inputEndRevision,
