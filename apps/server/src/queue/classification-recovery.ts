...
interface ClassificationRecoveryLogContext {
  photoId: string;
  provider: string;
  userId: string;
  taskId: string;
...
interface ClassificationRecoveryDependencies {
  ...
  retryStrategy: unknown;
  logger: {
    info: (context: ClassificationRecoveryLogContext, message: string) => void;
  };
}
...
export async function enqueueClassification(
  ...
  logger.info({ photoId, provider, userId, taskId: task.taskID }, "Classification recovery enqueued");
}
