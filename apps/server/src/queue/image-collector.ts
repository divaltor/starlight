...
export const imagesWorker = new Worker<ImageCollectorJobData>(
  ...
  async (job) => {
    ...
    logger.info({ tweetId: tweet.id, userId, photos: tweetRecord.photos.length }, "Tweet upserted with photos");
    ...
    for (const photo of tweetRecord.photos) {
      ...
      if (similarPhotos.length > 0) {
        ...
        try {
          ...
          logger.error({ err: error, photoId: photo.id, userId }, "Failed to enqueue classification job");
        }
...
imagesWorker.on("failed", (job) => {
  logger.error({ err: job?.failedReason, jobId: job?.id, stack: job?.stacktrace }, "Image collector job failed");
});
