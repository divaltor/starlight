...
import { readResponseBounded } from "./media-download";
import {
	buildFfmpegConcat,
	extractUgoiraZip,
...
describe("parsePixivArtworkUrl", () => {
	test("accepts only canonical HTTPS artwork URLs", () => {
		expect(parsePixivArtworkUrl("https://www.pixiv.net/artworks/12345")).toBe(
			"12345"
		);
		expect(parsePixivArtworkUrl("https://pixiv.net/artworks/12345/")).toBe(
			"12345"
		);
		expect(
			parsePixivArtworkUrl("http://www.pixiv.net/artworks/12345")
		).toBeNull();
		expect(
			parsePixivArtworkUrl("https://pixiv.net.evil.test/artworks/12345")
		).toBeNull();
		expect(
			parsePixivArtworkUrl("https://www.pixiv.net/users/12345")
		).toBeNull();
	});
});
...
describe("ugoira conversion input", () => {
	...
	test("rejects traversal in frame metadata", async () => {
		...
		expect(
			extractUgoiraZip(archive, [{ file: "../frame.jpg", delay: 100 }])
		).rejects.toThrow("Unsafe ugoira archive path");
	});
	...
	test("bounds declared and streamed download sizes", async () => {
		...
		await expect(readResponseBounded(declared, 10)).rejects.toThrow(
			"too large"
		);
		...
		await expect(readResponseBounded(streamed, 10)).rejects.toThrow(
			"too large"
		);
	});
	...
	test("bounds cumulative extraction and removes failed temp directories", async () => {
		const before = new Set(
			(await readdir(tmpdir())).filter((name) =>
				name.startsWith("starlight-ugoira-")
			)
		);
		const zip = new JSZip();
