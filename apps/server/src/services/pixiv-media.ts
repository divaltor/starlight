...
const writeBoundedEntry = async (entry: JSZip.JSZipObject, path: string, limit: number, total: { value: number }) => {
  const stream = entry.nodeStream("nodebuffer");
  const sink = Bun.file(path).writer();
  // JSZip exposes Node streams, so this promise is the adapter between stream
  // events and the async file writer.
...
export const parsePixivArtworkUrl = (value: string) => {
  ...
  const match = /^\/artworks\/(?<id>\d+)\/?$/u.exec(url.pathname);
  return match?.groups?.id ?? null;
};

export const buildFfmpegConcat = (frames: { file: string; delay: number }[]) => {
...
export const convertUgoira = async (concatPath: string, output: string) => {
  ...
  try {
    ...
    const outputStats = await Bun.file(output).stat();
    if (outputStats.size > MAX_PIXIV_DOWNLOAD_BYTES) {
