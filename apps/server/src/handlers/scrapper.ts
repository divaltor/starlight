...
import { FEED_SCRAPPER_QUEUE, scrapperQueue, SCHEDULED_SCRAPPER_INTERVAL_SECONDS } from "@/queue/scrapper";
...
type ProviderConnection = "connected" | "disconnected" | typeof LOOKUP_FAILED;

const getProviderConnection = (failed: boolean, connected: boolean): ProviderConnection => {
  if (failed) {
...
async function getScrapperConnections(ctx: Context): Promise<ScrapperConnections> {
  ...
  return {
    twitter: getProviderConnection(twitter.status === "rejected", twitter.status === "fulfilled" && twitter.value),
    pixiv: getProviderConnection(
      pixivCredential.status === "rejected",
      pixivCredential.status === "fulfilled" && pixivCredential.value?.credentialType === "refresh_token",
    ),
  };
}
...
async function startPixivCollection(
  ...
  if (schedule.status === "rejected") {
    logger.warn({ error: schedule.reason, userId, provider: "pixiv" }, "Failed to schedule collector");
  }
  if (immediate.status === "rejected") {
    logger.warn({ error: immediate.reason, userId, provider: "pixiv" }, "Failed to start collector");
  }
...
privateChat.command("scrapper", async (ctx) => {
  ...
  const hasConnectedProvider = connections.twitter === "connected" || connections.pixiv === "connected";
  ...
  if (hasConnectedProvider) {
    ...
    const results = [twitter && { provider: "Twitter", ...twitter }, pixiv && { provider: "Pixiv", ...pixiv }].filter(
      (result) => result !== null,
    );
    const lookupFailures = [
      connections.twitter === LOOKUP_FAILED && "• Twitter: connection check failed; collection was not started.",
      connections.pixiv === LOOKUP_FAILED && "• Pixiv: connection check failed; collection was not started.",
    ].filter((line) => line !== false);
    const complete =
      lookupFailures.length === 0 && results.every((result) => result.immediateStarted && result.scheduleReady);
    ...
    const lookupFailures = [
      connections.twitter === LOOKUP_FAILED && "• Twitter: connection check failed.",
      connections.pixiv === LOOKUP_FAILED && "• Pixiv: connection check failed.",
    ].filter((line) => line !== false);
