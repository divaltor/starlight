...
import { isTwitterUrl, Prisma, prisma } from "@starlight/utils";
...
const INLINE_QUERY_AUTHOR_REGEX = /(?<separator>^|\s)@(?<author>[A-Za-z0-9_]+)/gu;

const createInlineImageResultId = (provider: string, externalMediaId: string, userId: string): string => {
...
async function searchInlineImagesWithLegacyQuery(
  ...
  while (allPhotos.length < photoOffset + pageQueryLimit) {
    ...
    const tweets = await runInlineImageQuery(
      ...
      // Paging deliberately closes over the current offset and filters.
    ...
    for (const tweet of tweets) {
      for (const photo of tweet.media) {
        const dedupeKey = createInlineImageDedupeKey(photo.provider, photo.id, photo.userId, photo.perceptualHash);
...
function parseInlineImageQuery(query: string) {
  const authors = [...query.matchAll(INLINE_QUERY_AUTHOR_REGEX)]
...
// grammY's filter takes a predicate and a handler; unicorn interprets the
// second argument as Array#filter's thisArg.
// oxlint-disable-next-line unicorn/no-array-method-this-argument
composer.on("inline_query").filter(
  // oxlint-disable-next-line unicorn/no-array-method-this-argument
  (ctx) => !isTwitterUrl(ctx.inlineQuery.query.trim()),
  // The inline search intentionally keeps lexical, semantic, and recency
  // fallbacks in one update handler so Telegram receives one answer.
  // oxlint-disable-next-line eslint/complexity, sonarjs/cognitive-complexity
  async (ctx) => {
    ...
    const candidateLimit = Math.max((photoOffset + pageQueryLimit) * INLINE_QUERY_CANDIDATE_MULTIPLIER, 200);
    ...
    const authorFilter =
      authors.length > 0
        ? Prisma.sql`AND (${Prisma.join(
            authors.map(
              (author) => Prisma.sql`strpos(lower(COALESCE(t.author_username, t.username, '')), ${author}) > 0`,
            ),
            " OR ",
          )})`
        : Prisma.empty;
