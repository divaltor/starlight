      ...
      batch.map(async (mediaItem) => {
        try {
          ...
          if (!(metadata.height && metadata.width)) {
            logger.warn({ mediaId: mediaItem.id, userId: mediaItem.userId, metadata }, "Failed to extract dimensions");
          ...
          logger.error({ error, mediaId: mediaItem.id, userId: mediaItem.userId }, "Failed to update media dimensions");
          return "failed" as const;
        }
      }),
    );
    updated += results.filter((result) => result.status === "fulfilled" && result.value === "updated").length;
